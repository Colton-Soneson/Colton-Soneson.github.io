...
const bindGroupLayout_RGBA32FLOAT = device.createBindGroupLayout({
  label: "debugTexture Compute Bind Group Layout RGBA32FLOAT",
  entries: [{
    binding: 0,
    visibility: GPUShaderStage.COMPUTE,	//visibility is GPUShaderStage flags that indicate which shader stages can use resource
    buffer: {} //buffer key, other options are things like "texture" or "sampler", default is uniform, leave empty for binding 0
...
function createBindGroupsDebugTexture_RGBA32FLOAT(aCanvasTexture, aTextureToDebug) {
	const bindGroups = [
  device.createBindGroup({
    label: "debugTexture compute bind group RGBA32FLOAT",
    layout: bindGroupLayout_RGBA32FLOAT,
    entries: [{
      binding: 0,
      resource: { buffer: debugTextureUniformBuffer }	//buffer key, other options are things like "texture" or "sampler"
    },
...
const debugTexturePipelineLayout_RGBA32FLOAT = device.createPipelineLayout({
  label: "debugTexture compute Pipeline Layout RGBA32FLOAT",
...
const debugTexturePipeline_RGBA32FLOAT = device.createComputePipeline({
  label: "debugTexture pipeline RGBA32FLOAT",
